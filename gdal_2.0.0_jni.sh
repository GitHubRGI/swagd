...
if [ ! -d "$HOME/gdal/lib" ]; then
    ...
    cp java.opt gdal-2.0.0/swig/java
    # configure and make/install gdal
    echo 'Making GDAL...';
    cd gdal-2.0.0 && ./configure --prefix=$HOME/gdal_2.0.0 --without-libtool && make && make install;
    ...
    cd .. && mkdir -p $HOME/gdal_2.0.0/swig && cp -r java $HOME/gdal_2.0.0/swig;
    ln -s /usr/lib/libproj.so.0.7.0 $HOME/gdal_2.0.0/lib/libproj.so
else
